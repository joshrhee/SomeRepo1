...
class PSPNet(nn.Module):
    ...
    def __init__(
        ...
        if self.use_ppm:
            ...
            fea_dim = fea_dim + len(bins)*(fea_dim//len(bins))
    ...
    def __replace_conv_with_dilated_conv(self):
        ...
        for name, mod in self.layer3.named_modules():
            if 'conv2' in name and (mod.stride, mod.dilation, mod.padding) == (2, 1, 1):
                updates = [(name + ".stride", 1), (name + ".dilation", 2), (name + ".padding", 2)]
                update_attribues.append(updates)
        
        for updates in update_attribues:
            for val in updates:
                setattr(self.layer3, val[0], val[1])

        update_attribues = []
        for name, mod in self.layer4.named_modules():
            if 'conv2' in name and (mod.stride, mod.dilation, mod.padding) == (2, 1, 1):
                updates = [(name + ".stride", 1), (name + ".dilation", 4), (name + ".padding", 4)]
                update_attribues.append(updates)
        
        for up in update_attribues:
            for val in up:
                setattr(self.layer4, val[0], val[1])   
    ...
    def __create_classifier(self, in_feats: int, out_feats: int, num_classes: int) -> nn.Module:
        ...
        cls = nn.Sequential(
            nn.Conv2d(in_channels=in_feats, out_channels=out_feats, kernel_size=(3, 3), padding=1),
            nn.BatchNorm2d(num_features=out_feats),
            nn.ReLU(),
            nn.Dropout2d(p=self.dropout),
            nn.Conv2d(in_channels=out_feats, out_channels=num_classes, kernel_size=(1, 1))
        )
    ...
    def forward(
        ...
        x = self.layer1(x)
        x = self.layer2(x)
        aux_output = self.layer3(x)
        main_output = self.layer4(aux_output)
        
        if self.use_ppm:
            main_output = self.ppm(main_output)

        logits = self.cls(main_output)
