...
def get_model_and_optimizer(args) -> Tuple[nn.Module, torch.optim.Optimizer]:
    ...
    if args.arch == "SimpleSegmentationNet":
        ...
        params = [
            ...
            {'params': model.cls.parameters(), 'lr': 10*args.base_lr, 'momentum': args.momentum, 'weight_decay': args.weight_decay}
        ]
    else:
        model = PSPNet(layers=args.layers, zoom_factor=args.zoom_factor, pretrained=args.pretrained, num_classes=args.classes)
        params = [
            {'params': model.layer0.parameters(), 'lr': args.base_lr, 'momentum': args.momentum, 'weight_decay': args.weight_decay}, 
        ...
        if model.use_ppm:
            params.append({'params': model.ppm.parameters(), 'lr': 10*args.base_lr, 'momentum': args.momentum, 'weight_decay': args.weight_decay})
        params.append({'params': model.cls.parameters(), 'lr': 10*args.base_lr, 'momentum': args.momentum, 'weight_decay': args.weight_decay})
...
def update_learning_rate(current_lr: float, optimizer: torch.optim.Optimizer) -> torch.optim.Optimizer:
    ...
    for i, param in enumerate(optimizer.param_groups):
        if i < 5:
            ...
            param['lr'] = 10 * current_lr
        optimizer.param_groups[i] = param
...
def get_val_transform(args) -> transform.Compose:
    ...
    val_transform = transform.Compose([
        transform.ResizeShort(size=args.short_size),
        transform.Crop(size=(args.train_h, args.train_w), crop_type = "center", padding=mean, ignore_label=args.ignore_label),
        transform.ToTensor(),
        transform.Normalize(mean=mean, std=std)
    ])
