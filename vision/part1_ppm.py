...
class PPM(nn.Module):
    ...
    def __init__(self, in_dim: int, reduction_dim: int, bins: List[int]) -> None:
        ...
        for b in self.bins:
            self.features.append(
                nn.Sequential(
                    nn.AdaptiveAvgPool2d(output_size=b),
                    nn.Conv2d(in_channels=self.in_dim, out_channels=self.reduction_dim, kernel_size=(1, 1), bias=False),
                    nn.BatchNorm2d(num_features=self.reduction_dim, affine=False),
                    nn.ReLU(),
                )
            )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        N, in_dim, H, W = tuple(x.size())
        modules = torch.zeros((N, len(self.bins)*self.reduction_dim, H, W))
        for i, feature in enumerate(self.features):
            out = F.interpolate(feature(x), size=(H,W), mode='bilinear', align_corners=True)
            modules[:, i*self.reduction_dim:(i+1)*self.reduction_dim, :, :] = out
        
        C = self.in_dim + (len(self.bins) * self.reduction_dim)
